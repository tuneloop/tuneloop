...
async function route(req: IncomingMessage, res: ServerResponse, store: Store, dbPath: string): Promise<void> {
  ...
  if (path === '/api/sessions') {
    ...
    const reserved = new Set(['q', 'artifact', 'artifact_kind', 'limit', 'bucket', 'bucketval'])
    ...
    const bucket: Bucket | undefined =
      rawBucket === 'day' || rawBucket === 'week' || rawBucket === 'month' ? rawBucket : undefined
    sendJson(
      res,
      200,
      store.sessionList({
        ...
        bucket,
        bucketValue: q.get('bucketval') ?? undefined,
