...
function buildPrompt(session: Session, followups: Followup[], topics: FrictionTopicRef[]): { system: string; user: string } {
  ...
  const user = [
    ...
    '- rework: the user asked for previously produced work to be REDONE because it was wrong or unusable.',
    ...
    '- workflow progression ("commit and open a PR", "now do the next one", "mark it done"),',
    '- brand-new task requests and scope additions,',
    '- ordinary Q&A follow-ups where the user is learning, not correcting.',
    ...
    'occurrence of that same specific gap — do NOT force-fit an event into a popular topic; when unsure whether',
    'it matches, mint a new topic instead. Only propose new_topic_label for a pattern absent from the list.',
