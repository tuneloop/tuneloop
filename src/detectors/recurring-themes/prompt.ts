...
export function buildPrompt(session: Session, followups: Followup[], themes: ThemeRef[]): { system: string; user: string } {
  ...
  const user = [
    ...
    'Set FALSE (the default) for general agent- or user-behavior gaps that recur across any project (over-scoping,',
    'not verifying work, not surfacing its own capabilities, restating a personal convention) — those are global.',
