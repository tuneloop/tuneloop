...
const FLAGS: AutocompleteItem[] = [
  ...
  { value: "--config ", label: "--config <path>  JSON selecting which processors/detectors run" },
  { value: "--port ", label: "--port <n>  dashboard port (default 4319)" },
  { value: "--llm-provider ", label: "--llm-provider <name>  enrichment provider preset" },
  { value: "--llm-model ", label: "--llm-model <id>  enrichment model id (base/per-session)" },
  { value: "--llm-model-heavy ", label: "--llm-model-heavy <id>  stronger detector model (default: provider's strong sibling)" },
...
const HELP = [
  ...
  "the features you shipped are named, and cross-session recommendations are surfaced —",
  "using your own key. The reasoning-heavy recommendations run on a stronger 'heavy'",
  "model, auto-selected where the provider has one (override with --llm-model-heavy).",
  "You are asked once per analyze run, or set TUNELOOP_LLM_PROVIDER (+ key) to skip the",
  "prompt. Session summaries go only to the provider you choose.",
  "",
