 ...
 * unit tests assert against (deterministic when `nowMs` is fixed). The bottom of
 * this file is a CLI that seeds a throwaway sqlite file for manual UI eyeballing.
 */
...
// ---- CLI: seed a throwaway store for manual UI eyeballing ----
// Usage: npx tsx scripts/seed-skills.ts [dbPath]
//   then: npx tsx src/cli.ts serve --db <dbPath> --port 7799  →  open #/skills
// `nowMs` is stamped once at CLI time (fine here — this path is not the deterministic
// test path, which passes a fixed nowMs into seedSkillStore directly).
